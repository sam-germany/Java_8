...
public class Test_min {
	...
	public static void main(String[] args) {
		...
		System.out.println(l.stream().min((a,b) -> a>b ? a : b));  // hier we are not comparing all the values
		...
		System.out.println(l.stream().min(Integer::max)); // it return   Optional[2]  as  .max(Comparator)  and
		                                                  // comparator we need  2 arguments but hier 
		                                                  // (Integer::max) take only one argument so we get
		                                                  // the first value as output
		
		System.out.println(l.stream().min(Integer::compare));     // it return Optional[9]
		System.out.println(l.stream().min(Integer::compare).get());  // it return also Optional but with .get()
		                                                            // it return object  that is inside Optional
		
		System.out.println("################");
		
		System.out.println(l.stream().min((x,y) -> x.compareTo(y)));       
