...
class Player3 extends Thread{
     ...
     public Player3(CyclicBarrier cb) {
...
class Match3 implements Runnable{
...
public class Test3 {                                              // will be executed one time ,so 2 threads come one by one           
	public static void main(String[] args) {                      // or same time to cb.await() and after that 2 time 
		      ...
		      Match3 m1 = new Match3();                        
		      CyclicBarrier cb = new CyclicBarrier(2, m1);         
		                                                           
		      ExecutorService es = Executors.newFixedThreadPool(1);    // but as we have hier only 1 Thread so cb.await() method
		                                                             // is waiting for 2 Threads then only it can Trip the barrier
		      es.execute(new Player3(cb));                           // so it will keep waiting infinitely.
		      es.execute(new Player3(cb));                   
